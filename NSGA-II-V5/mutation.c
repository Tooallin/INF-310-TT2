...
void mutation_ind(individual *ind, problem_instance *pi) {
	int choice;
	double prob, prob_op, prob_perturb;

	prob = randomperc();
	prob_perturb = randomperc();

	if (prob <= pmut_bin) {
		if (prob_perturb <= 0.05) {
			choice = rnd(1, 2);
			if (choice == 1) route_ruin_mutation(ind);
			else shuffle_route_mutation(ind);
		} else {
			prob_op = randomperc();
...
/* Routine for route ruin mutation (Perturbation) */
void route_ruin_mutation(individual *ind) {
	int start, end, start_unvisited, end_unvisited;
	int route = rnd(1, n_routes);
	int i, k, j;
	...
	if (route_len < 3) return;

	int ruin_len = rnd(2, maximum(2, route_len / 2)); 
	int ruin_start = rnd(start, end - ruin_len + 1);

	int *new_gene = (int *)malloc(gene_length * sizeof(int));
	int *ruined_nodes = (int *)malloc(ruin_len * sizeof(int));

	for (i = 0; i < ruin_len; i++) {
		ruined_nodes[i] = ind->gene[ruin_start + i];
	}
...
/* Routine for route shuffle mutation (Perturbation) */
void shuffle_route_mutation(individual *ind) {
	int start, end;
	int route = rnd(1, n_routes);
