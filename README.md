 ...
 * **Elitist Route Crossover (ERC):** Al momento de heredar una ruta "élite" del primer padre, evalúa todas sus rutas alternando entre dos métricas (puntaje total o puntaje promedio por nodo). Guarda las 3 mejores rutas en la RCL y escoge una al azar para pasarla intacta a la descendencia, antes de completar el resto con el material genético del segundo padre.
 ...
 ## NSGA-II V5 Operadores de Perturbación
Incorpora operadores de destrucción o perturbación ("Ruin" y "Shuffle") diseñados para escapar de óptimos locales y romper el estancamiento (particularmente útil para los empates en instancias restrictivas). Estos operadores no buscan mejorar la solución de forma inmediata, sino destruir o alterar severamente una parte de ella para que los operadores heurísticos (*greedy*) tengan el espacio y margen de tiempo necesarios para reconstruirla de forma superior en las siguientes generaciones. Se integran con una probabilidad independiente y baja (5%) durante la fase de mutación para inyectar caos controlado sin destruir la convergencia global. Los nuevos operadores consisten en:
 * **Route Ruin Mutation:** Selecciona una ruta al azar y extrae un bloque contiguo de nodos (entre el 20% y el 50% de la longitud de la ruta), enviándolos de vuelta a la sección de "no visitados". Al vaciar una porción grande de la ruta, se genera un hueco de tiempo significativo que permite a operadores como *Greedy Insert* introducir nodos de alto valor que antes eran temporalmente inviables.
 * **Route Shuffle:** Selecciona una ruta activa y mezcla aleatoriamente el orden de todos sus nodos. Aunque esta acción suele penalizar temporalmente la eficiencia de la ruta (aumentando tiempos de viaje y espera), fuerza al algoritmo a salir de secuencias atascadas y a explorar topologías completamente nuevas para ese mismo clúster de POIs.