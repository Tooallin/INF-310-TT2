...
void selection(population *old_pop, population *new_pop, problem_instance *pi) {
	...
	for (i = 0; i < popsize; i+=4) {
		...
		if (prob <= pcross_bin) {
			...
			memcpy(new_pop->ind[i].gene, parent1->gene, gene_length * sizeof(int));
			memcpy(new_pop->ind[i+1].gene, parent2->gene, gene_length * sizeof(int));
		}
		...
		if (prob <= pcross_bin) {
			...
			memcpy(new_pop->ind[i+2].gene, parent1->gene, gene_length * sizeof(int));
			memcpy(new_pop->ind[i+3].gene, parent2->gene, gene_length * sizeof(int));
		}
	}
