...
/* Routine for greedy insert mutation */
void greedy_insert_mutation(individual *ind, problem_instance *pi) {
	...
	int best_u_idx = -1, best_pos = -1, attempt, pos;
...
/* Routine for replace/exchange mutation */
void replace_mutation(individual *ind, problem_instance *pi) {
	...
	int best_unv_idx = -1;
	int max_unv_score = min_score;
	
	for (int attempt = 0; attempt < 10; attempt++) {
		int u_idx = rnd(start_unv, end_unv);
		int u = ind->gene[u_idx];
		if (pi->set_POI[u-1].SCORE > max_unv_score) {
			max_unv_score = pi->set_POI[u-1].SCORE;
			best_unv_idx = u_idx;
		}
	}
